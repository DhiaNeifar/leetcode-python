        ...
        for index, char in enumerate(old_state):
            ...
            if char == '.':
                if (index == len(dominoes) - 1 and old_state[index - 1] == 'R') or (0 < index and old_state[index - 1] == "R") and (index < len(old_state) - 1 and old_state[index+ 1] != 'L'):
                    new_char = 'R'
                if (index == 0 and old_state[index + 1] == 'L') or (index < len(old_state) - 1 and old_state[index + 1] == 'L') and (0 < index and old_state[index - 1] != "R"):
                    new_char = 'L'
...
if __name__ == '__main__':
    dominoes_ = "."
    output = pushDominoes(None, dominoes_)
    print(output)
